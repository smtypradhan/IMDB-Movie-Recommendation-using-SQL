...
-- Average duration of movies in each genre? (Note: The same movie can belong to multiple genres.)
...
-- Minimum and maximum values in  each column of the rating table except the movie_id column
select * from ratings;
...
SELECTtitle, AVG(avg_rating) AS avg_rating,
RANK() OVER (ORDER BY AVG(avg_rating) DESC) AS avg_rating_rank
FROM movie INNER JOIN ratings ON movie.id = ratings.movie_id
GROUP BY title
limit 10;

/* Observations: Do you find your favorite movie FAN in the top 10 movies with an average rating of 9.6?
...
-- Hint: Finding null values for individual columns or follow below output format
SELECT 
...
/* Observations: James Mangold can be hired as the director for RSVP's next project. Do you remember his movies, 
...
/* Observations: Yes Marvel Studios rules the movie world.
